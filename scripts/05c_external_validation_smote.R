...
full_preprocessor <- readRDS(file.path(root, "results", "models", "full_preprocessor_smote.rds"))
...
threshold_defs <- read_csv_config(file.path(root, "results", "metrics", "internal_test_selected_thresholds_smote.csv"))
...
for (i in seq_len(nrow(successful))) {
  ...
  final_fit <- readRDS(file.path(root, "results", "models", sprintf("%s_smote_final_full.rds", model_name)))
  probs <- predict_probabilities(final_fit, x_external)
  bundle <- build_prediction_artifacts(y_external, probs, model_name, "external_validation_smote")
  ...
  if (nrow(model_thresholds) > 0) {
    threshold_rule_rows[[length(threshold_rule_rows) + 1]] <- apply_threshold_rules(
      ...
      dataset = "external_validation_smote",
      threshold_rules = model_thresholds
    )
  }
  save_performance_plots(y_external, probs, file.path(root, "results", "figures", sprintf("%s_external_smote", model_name)), sprintf("%s External Validation SMOTE", model_name))
}
...
utils::write.csv(external_metrics, file.path(root, "results", "metrics", "external_validation_metrics_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(external_topk, file.path(root, "results", "metrics", "external_validation_topk_metrics_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(external_predictions, file.path(root, "results", "metrics", "external_validation_predictions_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(external_threshold_rule_metrics, file.path(root, "results", "metrics", "external_validation_threshold_rule_metrics_smote.csv"), row.names = FALSE, na = "")

append_project_log(
  root,
  "05c External Validation With SMOTE",
  c(
    sprintf("- External validation rows scored with SMOTE-trained models: %s", nrow(external)),
    sprintf("- SMOTE models externally validated: %s", paste(successful$model_name, collapse = ", ")),
