...
split_obj <- readRDS(file.path(root, "results", "datasets", "internal_split_smote.rds"))
training_status <- read_csv_config(file.path(root, "results", "metrics", "training_status_smote.csv"))
successful <- training_status[training_status$status == "trained", , drop = FALSE]
if (nrow(successful) == 0) {
  stop("No trained SMOTE benchmark models are available for internal evaluation.")
}

train_preprocessor <- readRDS(file.path(root, "results", "models", "train_preprocessor_smote.rds"))
...
for (i in seq_len(nrow(successful))) {
  ...
  fit <- readRDS(file.path(root, "results", "models", sprintf("%s_smote_train_fit.rds", model_name)))
  probs <- predict_probabilities(fit, x_test)
  bundle <- build_prediction_artifacts(y_test, probs, model_name, "internal_test_smote")
  ...
  save_performance_plots(y_test, probs, file.path(root, "results", "figures", sprintf("%s_internal_smote", model_name)), sprintf("%s Internal Test SMOTE", model_name))
}
...
for (model_name in unique(internal_predictions$model_name)) {
  ...
  rules$dataset <- "internal_test_smote"
...
utils::write.csv(internal_metrics, file.path(root, "results", "metrics", "internal_test_metrics_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_topk, file.path(root, "results", "metrics", "internal_test_topk_metrics_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_threshold_grid, file.path(root, "results", "metrics", "internal_test_threshold_grid_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_threshold_defs, file.path(root, "results", "metrics", "internal_test_selected_thresholds_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_threshold_rule_metrics, file.path(root, "results", "metrics", "internal_test_threshold_rule_metrics_smote.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_predictions, file.path(root, "results", "metrics", "internal_test_predictions_smote.csv"), row.names = FALSE, na = "")

selected <- build_internal_selection_summary(internal_metrics, internal_topk, selection_col = "primary_model_smote")
utils::write.csv(selected, file.path(root, "results", "metrics", "model_selection_smote.csv"), row.names = FALSE, na = "")
...
saveRDS(full_preprocessor, file.path(root, "results", "models", "full_preprocessor_smote.rds"))
...
for (i in seq_len(nrow(successful))) {
  model_name <- successful$model_name[i]
  tuned_fit <- readRDS(file.path(root, "results", "models", sprintf("%s_smote_tuning.rds", model_name)))
  smote_full <- apply_simple_smote(x_full, y_full, k = 5, seed = 20260327 + i)
  final_fit <- fit_model_object(model_name, smote_full$x, smote_full$y, tuned_fit$best_params)
  saveRDS(final_fit, file.path(root, "results", "models", sprintf("%s_smote_final_full.rds", model_name)))
}
...
append_project_log(
  ...
  c(
    sprintf("- Internal SMOTE test metrics generated for: %s", paste(successful$model_name, collapse = ", ")),
    sprintf("- Selected primary SMOTE model: `%s`", selected$primary_model_smote[1]),
