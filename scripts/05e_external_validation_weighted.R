...
full_preprocessor <- readRDS(file.path(root, "results", "models", "full_preprocessor_weighted.rds"))
...
for (i in seq_len(nrow(successful))) {
  ...
  final_fit <- readRDS(file.path(root, "results", "models", sprintf("%s_weighted_final_full.rds", model_name)))
  probs <- predict_probabilities(final_fit, x_external)
  bundle <- build_prediction_artifacts(y_external, probs, model_name, "external_validation_weighted")
  ...
  if (nrow(model_thresholds) > 0) {
    threshold_rule_rows[[length(threshold_rule_rows) + 1]] <- apply_threshold_rules(
      ...
      dataset = "external_validation_weighted",
      threshold_rules = model_thresholds
    )
  }
  save_performance_plots(y_external, probs, file.path(root, "results", "figures", sprintf("%s_external_weighted", model_name)), sprintf("%s External Validation Weighted", model_name))
}
...
utils::write.csv(external_metrics, file.path(root, "results", "metrics", "external_validation_metrics_weighted.csv"), row.names = FALSE, na = "")
utils::write.csv(external_topk, file.path(root, "results", "metrics", "external_validation_topk_metrics_weighted.csv"), row.names = FALSE, na = "")
utils::write.csv(external_predictions, file.path(root, "results", "metrics", "external_validation_predictions_weighted.csv"), row.names = FALSE, na = "")
utils::write.csv(external_threshold_rule_metrics, file.path(root, "results", "metrics", "external_validation_threshold_rule_metrics_weighted.csv"), row.names = FALSE, na = "")

append_project_log(
  root,
  "05e External Validation With Class Weights",
  c(
    sprintf("- External validation rows scored with weighted models: %s", nrow(external)),
    sprintf("- Weighted models externally validated: %s", paste(successful$model_name, collapse = ", ")),
