...
if (nrow(successful) == 0) {
  stop("No trained benchmark models are available for internal evaluation.")
}

train_preprocessor <- readRDS(file.path(root, "results", "models", "train_preprocessor.rds"))
...
for (i in seq_len(nrow(successful))) {
  ...
  fit <- readRDS(file.path(root, "results", "models", sprintf("%s_train_fit.rds", model_name)))
  probs <- predict_probabilities(fit, x_test)
  bundle <- build_prediction_artifacts(y_test, probs, model_name, "internal_test")
  ...
  save_performance_plots(y_test, probs, file.path(root, "results", "figures", sprintf("%s_internal", model_name)), sprintf("%s Internal Test", model_name))
}
...
utils::write.csv(internal_metrics, file.path(root, "results", "metrics", "internal_test_metrics.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_topk, file.path(root, "results", "metrics", "internal_test_topk_metrics.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_threshold_grid, file.path(root, "results", "metrics", "internal_test_threshold_grid.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_threshold_defs, file.path(root, "results", "metrics", "internal_test_selected_thresholds.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_threshold_rule_metrics, file.path(root, "results", "metrics", "internal_test_threshold_rule_metrics.csv"), row.names = FALSE, na = "")
utils::write.csv(internal_predictions, file.path(root, "results", "metrics", "internal_test_predictions.csv"), row.names = FALSE, na = "")

selected <- build_internal_selection_summary(internal_metrics, internal_topk, selection_col = "primary_model")
utils::write.csv(selected, file.path(root, "results", "metrics", "model_selection.csv"), row.names = FALSE, na = "")
...
saveRDS(full_preprocessor, file.path(root, "results", "models", "full_preprocessor.rds"))
...
for (i in seq_len(nrow(successful))) {
  model_name <- successful$model_name[i]
  tuned_fit <- readRDS(file.path(root, "results", "models", sprintf("%s_tuning.rds", model_name)))
  final_fit <- fit_model_object(model_name, x_full, y_full, tuned_fit$best_params)
  saveRDS(final_fit, file.path(root, "results", "models", sprintf("%s_final_full.rds", model_name)))
}
...
append_project_log(
  ...
  c(
    ...
    sprintf("- Selected primary model: `%s`", selected$primary_model[1]),
